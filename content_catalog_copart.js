(() => {
  let isUpdating = false; // флаг защиты
  let lastFetchedIds = new Set(); // кэш последних ID
  ...
  async function updateBlocksWithApi() {
    if (isUpdating) {
      console.log("ExTKL: Skipping update (already running)");
      return;
    }
    isUpdating = true;
    ...
    try {
      ...
      // проверка: есть ли новые ID
      const newIds = uniqueIds.filter((id) => !lastFetchedIds.has(id));
      if (newIds.length === 0) {
        console.log("ExTKL: No new IDs, skipping API call");
        return;
      }

      console.log("ExTKL: Fetching info for new IDs", newIds);

      const extraInfo = await fetchExtraInfoViaBackground(newIds);
      ...
      lotBlocks.forEach((lotBlock) => {
        ...
        // обновляем только для новых
        if (!newIds.includes(lotId)) return;
      ...
      // обновляем кэш
      lastFetchedIds = new Set([...lastFetchedIds, ...newIds]);
    } catch (err) {
  ...
  async function initScript() {
    ...
    try {
      ...
      const observer = new MutationObserver(() => {
        if (isUpdating) return; // не перезапускать во время апдейта
  ...
  new MutationObserver(() => {
    if (location.href !== lastUrl) {
      ...
      lastFetchedIds.clear?.(); // сброс кэша при переходе
      initScript();
    }
